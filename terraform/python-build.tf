locals {
  ...
  python_lambda_layer_dir     = abspath("${local.python_src_dir}/${local.python_lambda_layer}")
  python_lambda_layer_zip     = "layer.zip"
  python_lambda_layer_zip_dir = abspath("${path.module}/../${local.python_src_dir}/${local.python_lambda_layer_zip}")
...
resource "aws_lambda_layer_version" "python_lambda_layer" {
  depends_on          = [docker_container.python_build_container, data.archive_file.python_lambda_layer_zip]
